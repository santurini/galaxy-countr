...
from util.galaxy_augmentation import TransformTrain
import models_mae_cross


# For this finetuning step we will use the same approach as it has been done in the paper. 
# In order to run their code, we keep their argument parser removing some commands and setting defaults
...
def get_args_parser():
    ...
    parser.add_argument('--data_path', default='../../data/GalaxiesV2', type=str,
                        help='dataset path')
    # we leave this two arguments as theirs as we will override them when running
    ...
    # set resume as bool so that we can try training from scratch
...
# train data is the same as their code
...
# note: we only adapted part of this code by changing some parameters,
# removing a few lines and so on. most of the sequent code aswell as parts of the comments
# come from CounTR itself
def main(args):
